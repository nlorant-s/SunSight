...
def scatter_plot(x, y, texts=None, xlabel="", ylabel="", title=None, fit=None, label="", show=True, color="palegreen", log=False, label_fit=True, alpha=0.1, fontsize=15, avgs=False, c=None, cmap=None, legend=False):
    ...
    if avgs:
        x_avg = np.mean(dat['x'])
        y_avg = np.mean(dat['y'])
        plt.hlines(y_avg, np.min(dat['x']), np.max(dat['x']), colors='darkgray' ,linestyles='dashed')
        plt.text(np.min(dat['x']), y_avg+15, "Average " + ylabel + ": " + str(np.round(y_avg, 2)), alpha=0.7, fontsize=8)
        plt.vlines(x_avg, np.min(dat['y']), np.max(dat['y']), colors='darkgray' ,linestyles='dashed') 
        plt.text(x_avg+5, np.max(dat['y']), "Average " + xlabel + ": " + str(np.round(x_avg, 2)), alpha=0.7, fontsize=8)
...
def complex_scatter(combined_df, x, y, xlabel, ylabel, fit=[1], title=None, bins=[], masks=[], show=True, states=None, legend=True, fontsize=15):
    '''
    Inputs:
