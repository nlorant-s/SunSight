...
projections = create_projections(combined_df, n=max_num_added, load=True, metric='yearly_sunlight_kwh_kw_threshold_avg')

panel_estimations_by_year = [("Net-Zero" , 479000 * 3), ("2030", 479000 * 1), ("2034", 479000 * 2)]
 
def plot_projections(projections, panel_estimations=None, net_zero_horizontal=False, interval=1, fontsize=20, fmts=["-X", "-H", "o-", "D-", "v-"], upper_bound='Greedy Carbon Offset'):
    ...
    if panel_estimations is not None:
        for label, value in panel_estimations:
            plt.vlines(value, np.array(projections[upper_bound])[-1]/18, np.array(projections[upper_bound])[-1], colors='darkgray' , linestyles='dashed', linewidth=2, alpha=0.5)
            plt.text(value - len(projections[upper_bound])/23, np.array(projections[upper_bound])[-1]/80, label, alpha=0.7, fontsize=16)
    ...
    plt.ylabel('Yearly Energy Generation (kWh)', fontsize=fontsize, labelpad=20)
    # plt.ylabel('Carbon Offset (metric tons)', fontsize=fontsize, labelpad=20)
    plt.legend(fontsize=fontsize/(1.5))
    plt.tight_layout()
    plt.show()

plot_projections(projections, panel_estimations_by_year, net_zero_horizontal=True, interval=100000, upper_bound='Greedy Average Sun')